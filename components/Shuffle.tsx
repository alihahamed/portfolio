...
const Shuffle: React.FC<ShuffleProps> = ({
  ...
  useGSAP(
    () => {
      ...
      const build = () => {
        ...
        const isGradient = className.includes('heading-gradient');
        const charExtraClass = isGradient ? ' heading-gradient' : '';
        ...
        chars.forEach(ch => {
          ...
          firstOrig.className =
            'text-left' + charExtraClass + ' ' + (shuffleDirection === 'up' || shuffleDirection === 'down' ? 'block' : 'inline-block');
          Object.assign(firstOrig.style, { width: w + 'px', fontFamily: computedFont });

          ch.setAttribute('data-orig', '1');
          ch.className =
            'text-left' + charExtraClass + ' ' + (shuffleDirection === 'up' || shuffleDirection === 'down' ? 'block' : 'inline-block');
          Object.assign((ch as HTMLElement).style, { width: w + 'px', fontFamily: computedFont });
  ...
  const cleanedClassName = useMemo(() => {
    return className.replace('heading-gradient', '').trim();
  }, [className]);

  const classes = useMemo(
    () => `${baseTw} ${ready ? 'visible' : 'invisible'} ${cleanedClassName}`.trim(),
    [baseTw, ready, cleanedClassName]
  );
  const Tag = tag || 'p';
