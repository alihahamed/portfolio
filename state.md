...
| 113| Resolve Hero title uppercase clipping     | Added container padding/margin offsets and expanded `.heading-gradient` box size to prevent mask bounds cutting letter tops | 2026-06-15 |
| 114| Background 3D scrolling text in About section | Implemented autoplaying circular scrolling text columns behind the document using GSAP ticker, optimized by rotating only the parent wrappers to reduce DOM updates from 48 to 2 per frame | 2026-06-15 |
